...
#  address: University of Osnabrück, Institute of Cognitive Science, Wachsbleiche 27, 49090 Osnabrück, Germany
#- name: Timo B. Roettger
#  affiliation: Universitetet i Oslo, Department of Linguistics and Scandinavian Studies, Niels Henrik Abels vei 36, 0313 Oslo, Norway
...
# loading the data
# set rmarkdown to read from project
...
guidelines  <-  read.csv("../../data/guidelines.csv")
coded_articles  <-  read.csv("../../data/Coding_Articles.csv", sep="|")
sample_journals <- read.csv("../../data/Sample_journals.csv")
...
# ! KK: shouldn't be deleted but rather added to their new journal names (LANGUAGE COGNITION AND NEUROSCIENCE, DIGITAL SCHOLARSHIP IN THE HUMANITIES)
df <- df %>% 
...
This sentiment expresses itself in two parts: Direct replications are claimed to be theoretically uninformative and conceptual replications are sufficient to assess the robustness of a field's empirical foundation [e.g., @stroebe2014alleged; @crandall2016scientific]. 
However, both these assumptions are unfounded [e.g., @zwaan_making_2018]: 
...
If the replication fails, that strong assumption was apparently incorrect, thus relevant auxiliary hypotheses must be reconsidered, which might weaken the theory.
Successful direct replication add important data to the discourse, allowing for more precise estimation of theoretically relevant parameters and thus allow to strengthen the derivation chain between theory and predictions [@meehl1990summaries].
Finally, additional evidence that a theoretically important finding can be confidently replicated is in itself theoretically relevant knowledge.

Thus direct replications are theoretically informative and a worthwhile endeavor. 
Conceptual replications on the other hand, i.e. replication attempts that have changed multiple critical design properties of the original study, can introduce interpretational ambiguity. 
...
Conceptual replications are often upheld as being more valuable than direct replication because they are assumed to simultaneously address concerns about reliability of an original claim and they are able to extend the original findings. 
Thus, conceptual replications are often considered sufficient for a field to move forward under the stipulation that repeated successful conceptual replications will occur only then when the prior research identified a true effect. 
However, there is increasing evidence that this stong assumption is empirically not supported. 
Issues related to questionable research practices [@john2012measuring], small sample sizes [@button2013power] and publication bias [@fanelli2012negative] can lead to a group of seemingly solid set of studies that appear to support an underlying theory, even though no single study from that set could survive a direct replication attempt. 
There are now a number of widely studied theories and effects that have been supported by dozens, if not hundreds of conceptual replications, but appear to crumble in light of meta-analyses or systematic replication attempts [e.g., @shanks2015romance; @wagenmakers2016registered]. 

In sum, direct replications are an under-appreciated tool to evaluate and cement the empirical and theoretical foundation of a field.
<!--TR: End of new section -->

The lack of replication studies threatens the very fabric of cumulative progress in experimental science because experimental results are often taken for granted without them ever being replicated. And this leads to a related problem: If we don't try, we won't fail to replicate our studies. And the recent past has shown that if we try, we fail more often than we would like to: Coordinated efforts to replicate published findings have uncovered alarmingly low rates of successful replications in fields such as psychology [@open_science_collaboration_estimating_2015], economics [@camerer_evaluating_2016], and social sciences [@camerer_evaluating_2018], a state of affairs that has been referred to as the “replication crisis” [@fidler_reproducibility_2018].
...
The study design has been preregistered at 2021-03-08 and can be inspected at [https://osf.io/a5xd7/](https://osf.io/a5xd7/) ^[Component has been made private to ensure double-blind peer review. See attached preregistration document for anonymized version.]. 
...
The full sample of journals can be inspected in the appendix of this article. ^[Two journals, namely "Language and Cognitive Processes" (since 2014: "Language, Cognition and Neuroscience") and "Literary and Linguistic Computing" (since 2015: "Digital Scholarship in the Humanities"), had to be excluded because it turned out during analysis that both journals have been renamed and that they have already been included in our sample under the new name. Our final sample thus included only 98 journals.]
...
We would like to stress that these three predictor variables do not reflect a temporal change. Instead, the currently retrievable data was used as a proxy in our preregistered aspiration to relate these factors to replication rates on the journal level.
...
Out of these articles, `r sum_repl` contained the term replicat\* which results to a mention rate of `r round(mention_rate * 100, 2)`\% across experimental linguistic articles.  
...
The distribution of the rate of replication mention substantially varies across journals ranging from `r min_rate` to `r max_rate`\%. The median rate of replication mention is as low as `r median_rate`\% (SD = `r std_rate`), a rate that is comparable to what @makel_replications_2012 have reported in their assessment of replications in psychology.
...
topten <- 
  ...
  scale_color_gradient(low = col_purple,
                       high = col_green) +
  scale_y_discrete(label = function(x) stringr::str_trunc(x, 20))+ # leave commented out
...
centered journal impact factors (continuous, henceforth jif), open access type (no, partial, DOAJ gold), and replication policies (binary: either explicitly encourage or not).\footnote{We diverted from the preregistered protocol in the following way after constructive exchanges with a reviewers: We originally planned to use uncentered jif and a binary version of open access as covariates. First, using ncentered journal impact factors would have provided estimates for journals with a journal impact factor of 0, which is less staightforward than using a mean journal impact factor of our sampled journals. Second, we our preregistered dichotomization of open access policy might obscure a possible ordinal relationship with open access policy and replication rate. Both the preregistered and revised models are available in the repository.}
...
predicted_values_cat <- mention_model %>%
  ...
  ### calculate contrasts
  mutate(DOAJgold = plogis(b_Intercept - b_openaccessDOAJgold),
         noOA = plogis(b_Intercept - b_openaccessno),
         policy = plogis(b_Intercept - b_binary_policy_s)) %>%
  ### transform into proportion space using the plogis function
...
## Material and methods
From the superset of 98 journals obtained above, the 20 journals with the highest proportion of experimental studies were selected for a more detailed analysis while excluding journals for which less than 2 hits (TS=replicat\*) could be obtained (see at [https://osf.io/f3yp8/](https://osf.io/f3yp8/) for a list of article counts per journal).^[Component has been made private to ensure double-blind peer review. See attached file 'Sample_articles.xlsx'.] 
Because of the skewed distribution of our sample (114 hits for Journal of Memory and Language, and less than 40 for all other journals, respectively), we randomly selected 50 out of the 114 articles for the Journal of Memory and Language (see at [https://osf.io/6vfpe/](https://osf.io/6vfpe/) for details).^[Component has been made private to ensure double-blind peer review. See attached file 'random_jml_articles.R' for anonymized version.] 
...
If the authors explicitly claimed that (one of) their research aim(s) was to replicate the result or methods of an initial study, this article was treated as a replication and was submitted to further analysis according to the preregistered coding scheme which can be inspected at [https://osf.io/ct2xj/](https://osf.io/ct2xj/).^[Component has been made private to ensure double-blind peer review. See attached file 'Coding_Sheet.xlsx' for anonymized version.] 
...
A change of experimental paradigm was easy to detect as the authors of a replication study usually state when they are investigating the same underlying effect as an initial study by using a different experimental paradigm, e.g. an oddball paradigm where previously an artificial grammar learning paradigm was used. Changes in the sample of a given study consisted mostly in conducting a study with native speakers of a different language to see whether the effect extends to a different language or language group. In other cases, the age group of the participants differed to see whether an effect generalizes across different age groups or identify the point in time where certain language skills ripen. A change in materials or experimental set-up has been identified when, e.g. the language under investigation changed, the authors had to construct their own material in line with the rationale of the initial study due to unavailability of the materials or when another self-reported change was made to the stimulus material or stimulus presentation either to rule out problems with the previous study design or to test the generalizability of the effect. 
All of the changes that have been identified by the manual coding procedure are changes that have been reported by the authors of the replication study. Most of these changes have been made by the authors to achieve a specific end by conducting the replication: Either show that an effect extends to another language, that it is robust across different experimental paradigms or subject groups or how different kinds of measurements, manipulations and controls affect the observed results. As such, we did not look for slight changes in the stimulus material like the correction of typos but changes that were identified have usually been changes of which the authors expected that they would change the results or improve the study in a significant way.
...
By analyzing the publication history of over 50000 articles across 98 journals that publish experimental linguistic research, our study found that `r round(mention_rate * 100, 2)`\% of experimental linguistic publications used the term replicat* in title, abstract or key words. 
...
All data and analyses are available online at [https://osf.io/9ceas/?view_only=1610b25ef9ad40468ee10e95dff30f9b](https://osf.io/9ceas/?view_only=1610b25ef9ad40468ee10e95dff30f9b).
...
<!--KK: CRediT roles are required according to website, but the manuscript should be anonymized.-->
