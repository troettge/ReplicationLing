...
We used Bayesian parameter estimation based on generalized linear regression models with a binomial link function.\footnote{A possible concern might be an inflation of zeroes if there are too many journals without a single mention of the search term. A zero-inflated binomial regression can account for such an inflation. We additionally ran a zero-inflated binomial model and the parameters are practically the same than the simpler binomial model. Both models are available in our materials.} 
...
Because of the skewed distribution of our sample (114 hits for Journal of Memory and Language, and less than 40 for all other journals, respectively), we randomly selected 50 out of the 114 articles for the Journal of Memory and Language in order to balance the sample (see at [https://osf.io/6vfpe/](https://osf.io/6vfpe/) for details).^[Component has been made private to ensure double-blind peer review. See attached file 'random_jml_articles.R' for anonymized version.] 
