...
pacman::p_load(rstudioapi, truncnorm, tidyverse, brms, ggdist, distributional, scales,
               ggstream, tidybayes, bayesplot, rticles, knitr, flextable)#, magick)
...
The distribution of the rate of replication mention substantially varies across journals ranging from `r min_rate` to `r max_rate`\%. The median rate of replication mention is `r median_rate`\% (SD = `r std_rate`), a rate that is comparable to what @makel_replications_2012 have reported in their assessment of replications in psychology.
...
All of the changes that have been identified by the manual coding procedure are changes that have been reported by the authors of the replication study. Most of these changes have been made by the authors in order to achieve specific goals: Either the aimed at showing that an effect extends to another language, that it is robust across different experimental paradigms or subject groups or how different kinds of measurements, manipulations and controls affect the observed results. As such, we did not consider slight changes in the stimulus materials like the correction of typos but only changes that were identified by the authors as expected to change the results or improve the study in a significant way.
