---
title: Amount and quality of direct replications in experimental linguistic research
author:
...
pacman::p_load(rstudioapi, truncnorm, tidyverse, brms, ggdist, distributional, 
               ggstream, tidybayes, bayesplot, rticles, knitr, ggalluvial, formattable, kableExtra, flextable)
...
The full sample of journals can be inspected at [https://osf.io/q2e9k/](https://osf.io/q2e9k/) or in the appendix of this article. <!--KK: include in appendix?--> <!--TR: Yes one pretty table!-->
...
They grouped psychology journals into four categories dependent on whether they (explicitly or implicitly) encourage replication studies or not in their "instructions to authors" and "aims and scope" sections on the journal websites. For our analysis, we only distinguished between those journals explicitly encouraging replication studies and those that do not. <!-- KK: explicitly state reason or is that obvious?--> 
...
```{r river_plot, fig.align = 'center', fig.height=5, fig.cap = "Quantity and quality of replications in experimental linguistics."}
...
# General discussion
The overall picture our analyses draw for replication attempts in the field of experimental linguistics is that they are far too uncommon. This relates to other fields of research for which a lack of replication studies has been attested [@makel_replications_2012; @makel_facts_2014; @open_science_collaboration_estimating_2015; @camerer_evaluating_2016; @camerer_evaluating_2018]. The median rate of replication mention of `r median_rate` (SD = `r std_rate`)`we find for experimental linguistics is in line with the rates found for psychology, social sciences and economics to name just a few.
...
The DOAJ gold standard with respect to open access publishing is also positively related to publishing replication studies. This might be due to journals which support open access are also more open to replication studies, both being important tools for the open science movement. <!--KK: JML also seems to have updated their journal policies in the meantime.-->
...
On the other hand the increasing amount of reproducibility initiatives founded in the field during the last few years give hope that the situation is likely going to change in the near future, e.g. many babies, many speeches etc. More and more journals offer registered reports and special issues dedicated to replications (e.g. Linguistics, Vol. 59: The replication crisis: Implications for linguistics). <!--KK: mention covid year 2020?-->
...
```{r journals_sample, tab.cap="The full sample of journals sorted by their ratio of experimental linguistics articles."}
...
flextable(sample_journals) # sometimes works, sometimes doesn't
```
