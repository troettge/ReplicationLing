...
pacman::p_load(rstudioapi, truncnorm, tidyverse, brms, ggdist, distributional, 
               ggstream, tidybayes, bayesplot, rticles, knitr, ggalluvial, flextable)
...
The counts for all articles/journals in the sample and calculated rates of replication mention can be inspected at [https://osf.io/yefr8/](https://osf.io/yefr8/). The journals coded for the factors journal policy, journal impact factors and open access publishing can be retrieved at [https://osf.io/rukc7/](https://osf.io/rukc7/). <!--KK: Todo: insert link with analyses-->
...
```{r topten-plot, fig.align = 'center', fig.width = 5, fig.height = 7, fig.cap = "Variation in rate of replicat* mention across those journals that exhibited at least one mention of the term. Numeric values on the right indicate the observed proportion of articles containing the string experiment* in title, abstract or keywords."}
...
ggplot(data = predicted_values, 
  ...
  scale_fill_gradient(low = col_purple,
                       high = col_green) +
  ylab("Empirical/predicted rate of replication mention") +
  ylim(0, 0.20) +
  xlim(0, 4) +
...
The stable rate of direct replication is somewhat surprising given that over the last ten years or so the concept of direct replication has received more and more attention. One possible reason for the fact that (direct) replication rates are not increasing for the field according to our analysis could be that experimental linguistics predominantly replicates experimental findings across languages, making the studies by definition only partial replications.
