...
From all journals that include at least one experimental linguistic study according to our criterium (418), journals with less than 100 published articles are excluded, yielding 259 remaining journals. 
...
Counts have been obtained on the 21st February 2021. See [osf-link] for more details.
...
We will use (weakly) informative normal priors centered on -2.1973 (corresponding to a 10% base rate, sd = 2.5) for the intercept since we expect very low base rates [e.g. @makel_replications_2012]. 
We will use weakly informative Cauchy priors centered on zero (scale = 2.5) for all population-level regression coefficients. 
...
We summarize these distributions by reporting the posterior mean and the 95% credible intervals (henceforth CrIs, calculated as the highest posterior density interval). 
...
This method yields a total number of 274 articles. 
Because of the skewed distribution of our sample (114 hits for Journal of Memory and Language, and less than 40 for all other journals), we randomly select 50 out of the 114 articles for the Journal of Memory and Language to achieve a more balanced distribution of papers across journals.
 <!-- TR: how is this random selection performed?-->
...
It then qualifies for further analysis after the coding scheme that can be viewed here: [osf-link]. 
...
An author overlap is attested when one of the authors is a (co-)author on both both articles.
...
We will use (weakly) informative normal priors centered on -2.1973 (corresponding to 10% base rate, sd = 2.5) for the intercept (since we expect very low base rates) and weakly informative Cauchy priors centered on zero (scale = 2.5) for all population-level regression coefficients. 
Four sampling chains with 2000 iterations each will be run for each model, with a warm-up period of 1000 iterations. 
